import React, { useRef, useState, useMemo, useEffect, useLayoutEffect, Fragment } from "react";
...
import RephrasingTeaser from "../rephrasing-teaser/rephrasing-teaser";
...
type AnyParaphraseType = string & { _type?: "paraphrase" };
type ParaphraseType = "formality" | "general" | "paraphrase" | "shortened" | "simplicity" | AnyParaphraseType;
...
const getAvailableFiltersFromPhrases = (paraphrasings: RephraseResult | undefined): ParaphraseType[] => {
	return uniq(
		paraphrasings?.rephrases
			...
			.filter((l): l is ParaphraseType => PARAPHRASING_TYPES.includes(l)) || []
	);
};
...
const _RemainingRephrasingsHint: React.FC<{ remainingRephrasings: number }> = ({ remainingRephrasings }) => {
	const getMessage = useI18nContext();
	const i18n = useMemo<
		Record<
...
const ParaphrasingContent: React.FC<Props> = ({
	...
	const [paraphrasings, isLoading, error] = useLoadParaphrasings({
		shouldLoadData: isEnabled && (remainingRephrasings === null || remainingRephrasings > 0),
		inhouseOnly,
		language,
		sentence,
		uniqueId,
	...
	useEffect(() => {
		TrackerAdapter.trackEvent("Action", "rephrasesV3:open", language + ":" + (isEnabled ? "enabled" : "disabled"));
	}, [language, isEnabled]);

	useEffect(() => {
		const filter = currentFilter || "default";
		TrackerAdapter.trackEvent("Action", "rephrasesV3:set_filter", filter);
	}, [currentFilter]);
	...
	useLayoutEffect(updateCardPosition, [paraphrasings, showAll, currentFilter, updateCardPosition]);

	useEffect(moveCardIntoViewport, [paraphrasings, showAll, currentFilter, moveCardIntoViewport]);
