...
import { isFunction, isNumber, isString, isUndefined, stripHTMLTags } from "../../../core/utils";
...
export interface Props {
	...
	onUnlockCard?: (onUnlockComplete: (isSuccess: boolean | Error) => Promise<void>) => void;
	error?: TextError;
...
export function isErrorContentProps(o: unknown): o is Props {
	return isObject(o) && "type" in o && o.type === "correct";
}

const ErrorFixButton: React.FC<{ fix: ErrorFix }> = ({ fix }) => {
	const label = useFixLabel(fix.type, fix.text);
	...
	const buttonProps = {
		className: classes(
			...
			[
				...
				"insert-en-dash",
				"remove-whitespace",
			].includes(String(fix.type)) && "lt-error-content__suggestion--optional",
...
const ErrorContentFooter: React.FC<
	Pick<Props, "isHiddenMatch" | "trialRemainingDays" | "onTurnOffPickyMode" | "onPremiumTeaserClick">
> = ({ isHiddenMatch, trialRemainingDays, onPremiumTeaserClick, onTurnOffPickyMode }) => {
	const getMessage = useI18nContext();
	const { current: i18n } = useRef<
		Record<
			| "popupTrialEndingInDays"
			| "popupTrialEndingTomorrow"
			| "popupTrialEndingToday"
			| "dialogPremiumButton"
		...
		>
	>({
		popupTrialEndingInDays: getMessage("popupTrialEndingInDays", [String(Math.round(trialRemainingDays || 0))]),
		popupTrialEndingTomorrow: getMessage("popupTrialEndingTomorrow"),
		popupTrialEndingToday: getMessage("popupTrialEndingToday"),
		dialogPremiumButton: getMessage("dialogPremiumButton"),
		errorCardHiddenMatchButton: getMessage("errorCardHiddenMatchButton"),
		errorCardHiddenMatchCaption: getMessage("errorCardHiddenMatchCaption"),
	...
	switch (true) {
		case typeof trialRemainingDays === "number" && trialRemainingDays < 6:
			if (trialRemainingDays > 2) {
				caption = stripHTMLTags(i18n.popupTrialEndingInDays);
			} else if (trialRemainingDays > 1) {
				caption = stripHTMLTags(i18n.popupTrialEndingTomorrow);
			} else {
				caption = stripHTMLTags(i18n.popupTrialEndingToday);
			}
			modifier = "lt-error-content__footer--trial";
			icon = "";
			buttonText = i18n.dialogPremiumButton;
			handleClick = (event) => {
				event.stopImmediatePropagation();
				onPremiumTeaserClick?.("addon2-error-card-trial-running");
			};
			break;
	...
	return (
		<CardContentFooter className={classes("lt-error-content__footer", modifier)}>
			<Div className="lt-error-content__footer__caption" onClick={buttonText ? undefined : handleClick}>
				{icon && <Icon name={icon} compact={true} />}
				{caption}
			</Div>
			{buttonText && (
